...
def enhancedPacmanFeatures(state, action):
    ...
    nearestFoodDist = 0
    if foodLocs:
        nearestFoodDist = min([util.manhattanDistance(pacmanPosition, food) for food in foodLocs])
    ...
    for each in ghostPositions:
        distance = min(distance, util.manhattanDistance(pacmanPosition,each))
    if distance:
        distance= 10.0/distance
  
    features['ghostDistance'] = distance 
    capsules = successor.getCapsules()
    cDistance = 'inf'
    cDistance = min(util.manhattanDistance(pacmanPosition,each),cDistance)
    if cDistance:
        cDistance = 10.0/cDistance
    
    features['capsules'] = cDistance
